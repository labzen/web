...
public final class ClassCreator {
  ...
  private static final String PROCESSOR_COMMENTS = "labzen web version: 1.1.0, generating: com.squareup:javapoet, based Java 21";
  ...
  public void create() {
    ...
    root.getFields().forEach(field -> {
      ...
      field.getAnnotations().forEach(ann ->
        fieldSpecBuilder.addAnnotation(buildAnnotationSpec(ann)));
  ...
  private String buildMethodBody(ElementMethod method, ElementField defaultFieldElement) {
    ...
    var parameterNames = method.getParameters().stream().map(ElementParameter::getName).collect(Collectors.joining(", "));

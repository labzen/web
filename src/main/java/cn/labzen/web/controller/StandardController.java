 ...
 * 继承本接口就无需再继承 {@link SimplestController} 了
 * <p>
 * 继承本接口的Controller，如果在业务上不需要某一个入口，例如：DELETE /resource/{id}，则将对应的方法注解一个 {@link Abandoned} 即可，参考 `示例 1`。
 * <p>
 * 本接口无法满足业务需求的场景下，开发者可像常规的 <b>Spring Controller Class</b> 定义一个入口方法。<br/>
 * 为了保证Controller的 `单一职责原则`，在使用 <b>Labzen Web</b> 组件定义的 Controller接口中，不允许出现 <b>default Result customMethod(...) { ... }</b> 这种自定义方法，进行复杂的代码处理。<br/>
 * 参考 `示例 2` 快速定义一个入口，该方法将调用 {@link BS} 业务逻辑层进行处理，请确保业务逻辑层的方法存在。<b>注意：请统一返回类型为Result</b>
 * <p>
 * 在Controller定义接口中，还有一些实现其他功能的注解，可参考：
 * <li>{@link Abandoned}：弃用API入口
 * <li>{@link Caching}：对API响应内容进行缓存
 * <li>{@link Call}：在方法上注解，标识将调用指定的其他 Component 的指定方法，如果不是特殊业务需要，应保持Controller和业务处理访问的关系一致性，所以不建议使用
 * <li>{@link Catching}：<b>Labzen Web</b> 组件有统一的异常信息处理，如需对某个入口产生异常时，进行特定的处理，可使用该注解
 * <li>{@link Crypto}：对API入参或响应内容进行加密配置
 * <li>{@link MappingVersion}：定义API入口的版本号
 * <li>{@link Monitor}：对API接口进行监控，配置监控项
 * <li>{@link Threshold}：调整入口的并发访问域值等相关参数及熔断方式
 * <p>
 ...
 * </code>
 *
 * @param <BS> Business Service Component Class: 指定当前Controller入口需要调用的业务逻辑处理类，一般为XXXService
 * @param <RB> Resource Bean - 指定当前Controller处理的资源Bean，所有的资源传递都将通过该Bean作为参数的包装容器
 * @param <ID> Resource Bean ID Type - 资源Bean的主键类型
 */

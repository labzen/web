...
public class ApiLogConfigLoader {
   ...
   *
   * @param registry 元数据注册表，用于解析哈希 key 对应的方法名
   * @return Map&lt;Controller接口名, Map&lt;方法Key, ApiLogConfig&gt;&gt;
   */
  public Map<String, Map<String, ApiLogConfig>> loadAll(LoggableControllerMetaRegistry registry) {
    ...
    for (Resource resource : resources) {
      try {
        ...
        if (registry.lookup(controllerName).isEmpty()) {
        ...
        if (methods != null) {
          for (Map.Entry<String, ApiEndpointLogConfig> entry : methods.entrySet()) {
            ...
            if (HASH_KEY_PATTERN.matcher(rawKey).matches()) {
              Optional<String> methodNameOpt = resolveMethodNameFromHash(registry, controllerName, rawKey);
        ...
        logger.info("已加载 API 日志配置: {} ({} 个方法级配置)", controllerName, controllerConfigs.size() - 1);
      } catch (Exception e) {
  ...
  private static void resolveCondition(ApiEndpointLogConfig config) {
    if (config.getConditionExpression() != null && !config.getConditionExpression().isBlank()) {
  ...
  private Optional<String> resolveMethodNameFromHash(LoggableControllerMetaRegistry registry, String controllerName, String hashKey) {
    return registry.lookup(controllerName)
        .flatMap(meta -> Optional.ofNullable(meta.methods().get(hashKey)))
        .map(ControllerMethodMeta::methodName);
  }

  private boolean isMethodExists(LoggableControllerMetaRegistry registry, String controllerName, String methodName) {
    return registry.lookup(controllerName)
        .map(meta -> meta.methods().containsKey(methodName))
        .orElse(false);
  }
